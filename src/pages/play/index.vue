<template>
<div class='' v-show="playlist.length>0">
    
    <div class="bg" v-show="fullScreen">
      <div class="bg1">
        <div class="filter"></div>
        <img :src="music.musicImg" alt="" width="100%" height="100%">
      </div>
    <van-nav-bar left-text="返回" :title="music.songName" left-arrow @click-left="onClickLeft" fixed />
    <div class="content">
      <div style="width:100%;height:50%">
            <img :src="music.musicImg" :class="this.audio.playing? 'zhuan':''" alt class="baImg" width="250"/>
      </div>
    ...
    <div class="control">
      ...
      <div class="playBtn">
          <img src="../../assets/img/shangyishou.png" alt="" class="conn" @click="up" >
              <!-- 音频播放控件 -->
          ...
          <img src="../../assets/img/xiayishou.png" alt="" class="conn" @click="next">
      </div>
      ...
      <!-- 播放列表 -->
      <van-popup v-model="show"  closeable round  close-icon="close" position="bottom" :style="{ height: '50%' }" >
        <!-- <div class="title">共{{playlist.length}}</div> -->
          <div class="pop" v-for="(song,index) in playlist" :key="song.id" @click="getSongIndex(index)" :class="currentIndex == index ? 'active':''"  > 
              <div class="count">{{index+1}}</div>
              <div class="songname">{{song.name}} 
                <span style="color:#666;font-size:12px"> - {{song.singer}}</span>
              </div>
          </div>
  ...
  <!-- 小型播放器 -->
  <div class="mini-player" v-show="!fullScreen" @click="showfullScreen" style="z-index=999">
        <div class="left">
          <img :src="music.musicImg" :class="this.audio.playing? 'zhuan':''" class="mini_img"  width="32" height="32"/>
        </div>
        <div class="text">
            <span class="tit">{{music.songName}}</span>
        </div>
...
export default {
  ...
  methods: {
    ...
    getcurrentSongIndex(){
      if(this.playlist.length>0){
        let currentSongDetail= this.playlist[this.currentIndex]
        console.log(currentSongDetail) 
        this.musicid=currentSongDetail.id;
        this.getSong();
        this.SongDetail();
       }
    },
    ...
    saveCurrent(){
     this.$store.commit('set_music',this.musicdecs)
    },
    //改变播放方式
    changWay(){
      if(this.audioWay>1){
    ...
    onClickLeft() {
      ...
      let chrrentPath=this.$route.path
      if(chrrentPath=='/'){
      ...
      //  console.log('playing'+this.audio.playing)
    },
    // 播放跳转
    ...
    async SongDetail() {
      let res = await getSongDetail(this.musicid);
      ...
      this.musicdecs.songName=res.data.songs[0].name
      // console.log(res.data.songs[0].al)
      // console.log(this.musicid)
      this.$store.commit('storeCurrentSong',obj)
      this.musicdecs.musicImg = res.data.songs[0].al.picUrl;
    },
    async getSong() {
      let res = await getSongid(this.musicid);
      //  console.log(res.data);
      this.musicdecs.MusicSrc = res.data.data[0].url;
    },
    ...
    onTimeupdate(res) {
      ...
      this.$store.commit('set_showTime',this.sliderTime)
  ...
  currentIndex(newVal,oldVal){
    // console.log(newVal)
    var a=this.playlist[this.currentIndex]
    // console.log(a)
    this.$store.commit('set_record',a)
    this.getcurrentSongIndex()
    this.saveCurrent()
  }
  }
